...
def init(x, y, Tn=90, Tb=0, v=v0):  # Initialization of the printing procedure. All printing processes
    # should start with this function
...
def close(filename, x, y, z, Tn=0, Tb=0, s=0, v=v0):
    ...
    f.write("\n(Closing)\n")
    f.write("G01 \tX{:.2f} \tY{:.2f} \tZ{:.2f} \tF{}\n".format(x_abs + x, y_abs + y, z_abs + z,
                                                               vf))  # move to the finishing position
    f.write("M104 S{}\n".format(Tn))  # set nozzle temperature to Tn
    f.write("M140 S{}\n".format(Tb))  # set bed temperature to Tb
    f.close()
...
def film(l, w, h, v=v0):  # print film with parallel orientation in x axis
    ...
    for i in range(lay):
        ...
        z_abs += dz
        y_abs = y0 + (2 * ((
                                   z_abs / dz) % 2) - 1) * dxy / 2  # move half a thickness of the line up or
        # down depending on layer number
        f.write("G01 \tY{:.2f} \tZ{:.2f} \tF{}\n".format(y_abs, z_abs,
                                                         vf))  # move a layer up and down to a starting y position
...
def concentric_circle(d,
                      v=v0):  # outdated function; archimedean is the better one; print concentric circles starting at
    # (x_abs, y_abs, z_abs)
    global x_abs, y_abs, z_abs, l_tot
    ...
    for i in range(n + 1):
        ...
        f.write("G02 \tX{:.2f} \tY{:.2f} \tF{} \tE{:.5f} \tI{:.2f} \tJ{:.2f}\n".format(x_abs + r_temp, y_abs, v,
                                                                                       math.pi * r_temp * ext, r_temp,
                                                                                       0))
        f.write("G02 \tX{:.2f} \tY{:.2f} \tF{} \tE{:.5f} \tI{:.2f} \tJ{:.2f}\n".format(x_abs - r_temp, y_abs, v,
                                                                                       math.pi * r_temp * ext, -r_temp,
                                                                                       0))
...
def archimedean_spiral(d, slices=4, alpha=0, v=v0):  # print archimedes spiral approximation starting at
    # (x_abs, y_abs, z_abs)
    global x_abs, y_abs, z_abs, l_tot
    a = 2 * math.pi * alpha
    ...
    rot = np.array([[cos(a), sin(a), 0, 0, 0], [-sin(a), cos(a), 0, 0, 0],
                    [0, 0, cos(a), sin(a), 0], [0, 0, -sin(a), cos(a), 0],
                    [0, 0, 0, 0, 1]])  # define rotation matrix to allow to start from any given angle
    ...
    length = 2 * math.pi * np.sum(arcs[:, 4]) / slices
    l_tot += length
    ...
    f.write("G90 \n")
    f.write("M83 \n")
    f.write("G01 \tX{:.2f} \tY{:.2f} \tF{}\n".format(x_abs + cos(a) * arcs[0, 4], y_abs + sin(a) * arcs[0, 4],
                                                     vf))  # move to starting position

    for i in range(arcs.shape[0]):
        s = arcs[i]
        f.write("G03 \tX{:.2f} \tY{:.2f} \tF{} \tE{:.5f} \tI{:.2f} \tJ{:.2f}\n".format(s[0], s[1], v, ext * s[
            4] * 2 / slices * math.pi, s[2], s[3]))
    f.close()
    return length


def archimedean_spiral_3d(h, fnc, args, slices=4, alpha=0,
                          v=v0):  # print 3D archimedes spiral approximation starting at (x_abs, y_abs, z_abs)
    global x_abs, y_abs, z_abs, l_tot
    a = 2 * math.pi * alpha  # calculate standard angle from normalized one
    arcs = supp.arch_3d(h, fnc, args,
                        slices)  # calculate archimedes spiral starting at (0, 0) coordinates by cutting each circle
    # into slices
    ...
    for i in range(arcs.shape[0]):
        ...
        f.write("G03 \tX{:.2f} \tY{:.2f} \tZ{:.2f} \tF{} \tE{:.5f} \tI{:.2f} \tJ{:.2f}\n".format(
            s[0], s[1], s[2], v, ext * s[5] * 2 / slices * math.pi, s[3], s[4]))
    f.close()
    return length
...
def radial(d, v=v0):
    ...
    length = np.sqrt((lines[:, 1] - lines[:, 3]) ** 2 + (
            lines[:, 2] - lines[:, 4]) ** 2)  # calculate the length of each of the lines
    l_tot += np.sum(length)  # calculate total printing length
    ...
    for i in range(int(lines.shape[0] / 2)):
        ...
        f.write("G01 \tZ{:.2f} \tF{}\n".format(z_abs, vf))
        f.write("G01 \tX{:.2f} \tY{:.2f} \tF{} \tE{:.5f}\n".format(lines[2 * i, 1], lines[2 * i, 2], v,
                                                                   length[i] * ext))
        f.write("G01 \tZ{:.2f} \tF{}\n".format(z_abs + 1, vf))
        f.write("G01 \tX{:.2f} \tY{:.2f} \tF{}\n".format(lines[2 * i + 1, 1], lines[2 * i + 1, 2], vf))
        f.write("G01 \tZ{:.2f} \tF{}\n".format(z_abs, vf))
        f.write("G01 \tX{:.2f} \tY{:.2f} \tF{} \tE{:.5f}\n".format(lines[2 * i + 1, 3], lines[2 * i + 1, 4], v,
                                                                   length[i] * ext))
        f.write("G01 \tZ{:.2f} \tF{}\n".format(z_abs + 1, vf))
    f.close()
